...
export default function ModItem({ name, isModHaveConflict, isOneshotMod, modPath, iconBase64, author }: Prop): JSX.Element {
    ...
    return (
        <div className={`mod-item ${isOneshotMod ? isModHaveConflict ? "mod-item-invalid" : "" : "mod-item-invalid"}`}>
            {isOneshotMod ? <></> : <p className="mod-item-invalid-text">This is not a oneshot mod</p>}
            {isModHaveConflict ? <p className="mod-item-invalid-text">This mod conflict with other mod</p> : <></> }
