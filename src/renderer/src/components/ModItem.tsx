...
export default function ModItem(props: Prop): JSX.Element {
    ...
    useEffect(() => {
        async function setup(): Promise<void> {
            ...
            // filter the mod enable config setting to this spesific mod
            const result = modEnabledConfigs.filter(modEnabledConfig => {
                return modEnabledConfig.key === props.modPath
            })

            if (result.length != 0) {
                setEnabled(result[0].enabled);
                await window.api.setModEnabled(props.modPath, result[0].enabled);
            }
        }
    ...
    async function enableMod(): Promise<void> {
        setEnabled(!enabled);
        await window.api.setModEnabled(props.modPath, !enabled);
    ...
    async function openModFolderInFileManager(): Promise<void> {
        await window.api.openFolderInFileManager(props.modPath);
    }
    ...
    async function deleteMod(): Promise<void> {
        await window.api.deleteMod(props.modPath);

        // update the this mod enable config
        const settings: SettingsData = JSON.parse(await window.api.readSettingsFile());

        let modEnabledConfigs: EnableData[] = settings.modEnabledConfigs;

        modEnabledConfigs = modEnabledConfigs.filter((object) => {
            return object.key !== props.modPath;
        });
    ...
    return (
        <div className={"mod-item"}>
            ...
            <Modal haveCloseButton={false} canClose={false} className="mod-item-delete-warning" openModal={openDeleteModConfirm} closeModal={() => setOpenDeleteModConfirm(false)}>
                <p className="mod-item-delete-warning-text">Do you want to delete the mod folder at {props.modPath.trim() ? `"${props.modPath.trim()}"` : '""'}?</p>
